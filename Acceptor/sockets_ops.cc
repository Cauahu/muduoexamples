...
void sockets::listenOrDie(int sockfd){
	int ret = ::listen(sockfd, SOMAXCONN);//SOMAXCONN定义了系统中每一个端口最大的监听队列长度，默认是128
	if(ret < 0)
		LOG_SYSFATAL << "sockets::listen()" ;
}
...
int sockets::accept(int sockfd, struct sockaddr_in* peerAddr){
	...
	int connfd = ::accept4(sockfd, sockaddr_cast(peerAddr), &len, SOCK_NONBLOCK | SOCK_CLOEXEC);
	if(connfd < 0)
...
void sockets::fromHostPort(const char* ip, uint16_t port,struct sockaddr_in* addr){
	...
	if(::inet_pton(AF_INET, ip, &(addr->sin_addr)) <= 0)//inet_pton将点分十进制转换成二进制
