...
void TcpConnection::sendInLoop(const std::string& message){
    ...
    if(!channel_->isWriting() && outputBuffer_.readableBytes() == 0){
        ...
        if(nwrote >= 0){
            ...
            else if(remaining == 0 && writeCompleteCallabck_){
                loop_->queueInLoop(
                        boost::bind(writeCompleteCallabck_, shared_from_this()));
            }
        }
        else{
            nwrote = 0;
    ...
    assert(nwrote >= 0);
    if(muduo::implicit_cast<size_t>(nwrote) < message.size()){
        remaining = outputBuffer_.readableBytes();
        size_t oldLen = message.size() - nwrote;
