...
class ChatClient : boost::noncopyable
{
	public :
		ChatClient(EventLoop* loop, const InetAddress& serverAddr)
			...
			codec_(boost::bind(&ChatClient::onStringMessage, this, _1, _2, _3)){
				client_.setConnectionCallback(boost::bind(&ChatClient::onCennection, this, _1));
	...
	private :
		void onCennection(const TcpConnectionPtr& conn)
		{
		...
		void onStringMessage(const TcpConnectionPtr& conn,
				...
				Timestamp)
		{
			printf("%s: %s\n", conn->localAddress().toIpPort().c_str(),  message.c_str());
